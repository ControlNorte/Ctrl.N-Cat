...
def salvar_regra_usando_movimentacao(request):
    ...
    if request.method == 'POST':
        ...
        valor = request.POST.get('valor')
        valor = float(valor.replace('.', '').replace(',', '.'))
        regras = Regra.objects.create(tenant=request.tenant, cliente=dadoscliente,
                                      categoria=Categoria.objects.get(id=categoria, cliente=dadoscliente),
...
def importar_subcategorias(arquivo_importacao_cliente, tenant):
    ...
    if categorias_nao_encontradas:
        print("Categorias Mãe inexistentes:")
        for c in categorias_nao_encontradas:
            print("-", c)
